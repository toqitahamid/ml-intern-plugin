...
3. **Plugin root exists:** `ls "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/plugins/cache/ml-intern-plugin/ml-intern/0.1.0}" | head -5`
4. **Plugin venv:** `ls "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/plugins/cache/ml-intern-plugin/ml-intern/0.1.0}/.venv/bin/python" 2>&1`
...
7. **MCP server importable:** `cd "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/plugins/cache/ml-intern-plugin/ml-intern/0.1.0}" && uv run --no-sync python -c "from ml_intern_mcp.server import REGISTERED_TOOL_NAMES; print(f'{len(REGISTERED_TOOL_NAMES)} tools registered')" 2>&1`
